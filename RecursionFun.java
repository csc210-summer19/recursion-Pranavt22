 ...
 *
 * @author Rick Mercer and Pranav Talwar
 */
public class RecursionFun {
	...
	private int findMin(int[] a, int num) {
		if (num == 1) {
			return a[0];
		} else {
			return Math.min(a[num - 1], findMin(a, num - 1));
		}

	}

	private int findMax(int[] a, int num) {
		if (num == 1) {
			return a[0];
		} else {
			return Math.max(a[num - 1], findMax(a, num - 1));
		}
	}
